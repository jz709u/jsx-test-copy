...
class _DbSectionState extends State<_DbSection> {
  ...
  Future<void> _run(String id, Future<void> Function() action) async {
    ...
    setState(() { _loading.add(id); _result.remove(id); });
    try {
      await action();
...
class _FlightStatusPickerState extends State<_FlightStatusPicker> {
  ...
  Future<void> _setStatus(String flightId, String status) async {
    ...
    try {
      ...
      setState(() { _flightsFuture = widget.actions.getFlights(); });
    } catch (e) {
      if (mounted) {
        ...
        children: [
          ...
          FutureBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (_, snap) {
              ...
              return Column(
                children: flights.map((f) => _FlightStatusRow(
                  flight: f,
                  updating: _updating == f['id'],
                  onStatusTap: (status) => _setStatus(f['id'] as String, status),
                )).toList(),
              );
            },
          ),
...
class _FlightStatusRow extends StatelessWidget {
  ...
  const _FlightStatusRow({required this.flight, required this.updating, required this.onStatusTap});
        ...
        children: [
          Row(
            children: [
              ...
              Text('$origin → $dest', style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
              const Spacer(),
              if (updating)
            ...
            children: flightStatuses.map((s) => GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: s == status ? AppColors.gold.withValues(alpha: 0.15) : AppColors.surface,
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(color: s == status ? AppColors.gold.withValues(alpha: 0.6) : AppColors.divider),
                ),
                child: Text(s, style: TextStyle(color: s == status ? AppColors.gold : AppColors.textSecondary, fontSize: 11, fontWeight: FontWeight.w600)),
              ),
            )).toList(),
          ),
        ],
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
        children: [
          ...
          Text(label, style: const TextStyle(color: AppColors.white, fontSize: 14, fontWeight: FontWeight.w700)),
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(child: Text(mode.label, style: TextStyle(color: selected ? AppColors.gold : AppColors.white, fontSize: 14, fontWeight: selected ? FontWeight.w600 : FontWeight.w400))),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(label, style: const TextStyle(color: AppColors.white, fontSize: 13, fontWeight: FontWeight.w500)),
...
class _StatusChip extends StatelessWidget {
  ...
  Color get _color {
    switch (status) {
      ...
      case 'boarding':  return AppColors.gold;
      case 'departed':  return AppColors.textSecondary;
      case 'landed':    return AppColors.textSecondary;
